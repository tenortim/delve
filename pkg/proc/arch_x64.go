...
const (
	AMD64crosscall2SPOffsetBad        = 0x8
	AMD64crosscall2SPOffsetWindows    = 0x118
	AMD64crosscall2SPOffsetNonWindows = 0x58
)
...
func (a *AMD64) FixFrameUnwindContext(fctxt *frame.FrameContext, pc uint64, bi *BinaryInfo) *frame.FrameContext {
	...
	if a.crosscall2fn != nil && pc >= a.crosscall2fn.Entry && pc < a.crosscall2fn.End {
		rule := fctxt.CFA
		if rule.Offset == AMD64crosscall2SPOffsetBad {
			switch a.goos {
			case "windows":
				rule.Offset += AMD64crosscall2SPOffsetWindows
			default:
				rule.Offset += AMD64crosscall2SPOffsetNonWindows
			}
		}
		fctxt.CFA = rule
	}
